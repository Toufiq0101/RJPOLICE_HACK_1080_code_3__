...
pnet, rnet, onet = load_mtcnn_models()



def facial_data_extraction(video_path, save_dir, frame_skip=5):
    faces_in_video = []
    ...
    for i in range(0, frame_count, frame_skip):
        ...
        if frame.shape[-1] == 3 and frame.shape[-2] == 3:
            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
        else:
            frame_rgb = frame

        faces = []
        for face_info in pnet.detect_faces(frame_rgb):
            x, y, width, height = face_info["box"]
            box = [x, y, x + width, y + height]
            faces.append(box)

        for j, box in enumerate(faces):
            save_path = os.path.join(save_dir, f"frame_{i}_face_{j}.png")

            extract_face(Image.fromarray(frame_rgb), box, save_path=save_path)
        ...
        cv2.imshow("Video with Face Detection", frame_rgb)
...
video_path = "D:/ProjectX/number_plate/test_3.mp4"
save_directory = "D:/ProjectX/face1/facial_details"
facial_data_extraction(video_path, save_directory, frame_skip=5)
